...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  ...
  const navItems = [
    ...
    { name: 'AI', icon: 'smart_toy', path: '/ai' },
  ...
  const handleLogout = async () => {
    try {
      // Drop the cached dataset first: it is shared across the tab, so leaving
    ...
    <>
      ...
      >
        ...
        <nav className="flex-1 px-4 py-4 space-y-1.5 overflow-y-auto custom-scrollbar">
          {navItems.map((item) => {
            // Entries without a path are not built yet: render them as inert
            // rows instead of links that navigate nowhere.
            if (!item.path) {
              return (
                <div
                  key={item.name}
                  aria-disabled="true"
                  className="flex items-center px-4 py-3 rounded-lg text-slate-400 opacity-50 cursor-not-allowed select-none"
                >
                  <Icon name={item.icon} className="mr-3" />
                  <span className="font-medium">{item.name}</span>
                  <span className="ml-auto text-[10px] font-bold uppercase tracking-wider text-slate-400 bg-white/5 border border-white/10 rounded-full px-2 py-0.5">
                    Soon
                  </span>
                </div>
              );
